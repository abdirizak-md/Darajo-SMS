...
const SidebarComponent = ({ title, open, items }) => {
  ...
  return (
    <div className="mb-4">
      {open && ( <p className="px-6 mb-3 text-[10px] tracking-[0.25em] text-white/50 font-semibold"> {title} </p> )}
        ...
        {items.map((item) => (
          <NavLink key={item.label} to={item.path}
            ...
            className={({ isActive }) =>
              ...
              ${ isActive ? 'bg-white/15 text-white shadow-lg backdrop-blur-md' : 'text-white/70 hover:bg-white/10 hover:text-white' } ` } >
